...
def cleanStrRepr(string):
    return repr(string).lstrip('u').strip("'")


def notify(heading, message='', icon='', timeout=5000, sound=False):
    xbmcgui.Dialog().notification(heading, message, icon, timeout, sound)
