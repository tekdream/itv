...
DEFAULTSWFURL = 'http://www.rte.ie/static/player/swf/omsf2_2014_06_03_v12.swf'
PAGEURL = 'http://www.rte.ie/player/'
flashJS = "http://static.rasset.ie/static/player/js/flash-player.js"
# URL Constants
KNOWN_RTE_SHOWS_URL  = 'http://xbmc-vodie.googlecode.com/svn/trunk/plugin.video.vodie/xml/rteshows.json'
LIVE_LISTINGS    = 'http://feeds.rasset.ie/livelistings/'
LIVE_NOW_NEXT    = LIVE_LISTINGS + 'nownext/'
LIVE_PLAYLIST_URL = LIVE_LISTINGS + 'playlist?channelid=%s&platform=legacy'
PROGRAMME_URL    = 'http://feeds.rasset.ie/rteavgen/player/programme/?id='
SHOW_BY_TYPE_URL = 'http://feeds.rasset.ie/rteavgen/player/%s/'
PLAYLIST_URL     = 'http://feeds.rasset.ie/rteavgen/player/playlist?showId=%s&type=iptv1&time=%s'
FEATURES         = 'features'
LATEST           = 'latestfront'
...
# Channel Constants
CHANNEL = 'RTE'
LOGOICON = 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/rteplayer.png'
...
FEATMENU =      {'Thumb'    : 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/Features.png',
                 'Channel'  : CHANNEL,
                 'Title'    : 'Features'            ,
                 'url'      : FEATURES      ,
                 'mode'     : MenuConstants.MODE_CREATEMENU ,
...
LTSTMENU =      {'Thumb'    : 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/latest.png',
                 'Channel'  : CHANNEL,
                 'Title'    : 'Latest',
                 'url'      : LATEST,
                 'mode'     : MenuConstants.MODE_CREATEMENU ,
...
LASTMENU =      {'Thumb'    : 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/last%20chances.png',
                 'Channel'  : CHANNEL,
                 'Title'    : 'Last Chance',
                 'url'      : LASTCHANCE,
                 'mode'     : MenuConstants.MODE_CREATEMENU ,
...
ATOZMENU =      {'Thumb'    : 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/searchaz.png',
                 'Channel'  : CHANNEL,
                 'Title'    : 'Search by A-Z',
...
CATYMENU =      {'Thumb'    : 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/searchcat.png',
                 'Channel'  : CHANNEL,
...
class RTE:
    ...
    def getShows(self,type, params=''):
        if type == 'live':
            page = urllib2.urlopen(LIVE_NOW_NEXT + params)
        else:
            page = urllib2.urlopen(SHOW_BY_TYPE_URL%(type) + params)
        self.soup = BeautifulSoup(page, "html5lib", selfClosingTags=['link','category','media:player','media:thumbnail',
                                                              'rte:channel'])
        page.close()
        ...
        for item in items:
            ...
            if type == "live":
                ...
                if item.find(term="NOW"):
                    url = LIVE_PLAYLIST_URL % (self.getStringFor(item, 'rte:channel', 'id'))
                    yield { "id": id, "title":title, "thumb":str(pic), "url": url}
            else:
    ...
    def getStringFor(self, parent, tagName, attrName = None, default = 'None'):
        if parent.find(tagName):
            if attrName is None:
                print parent.find(tagName)
                try:
                    result = str(parent.find(tagName).string).strip()
                except:
                    result = ""
                return result
            else:
    ...
    def updateSWFURL(self):
        f = urllib2.urlopen(flashJS)
        text = f.read()
        f.close()

        try:
            match=re.compile('"http://www.rte.ie/static/player/swf/(.+?).swf"').findall (text)[0]
            self.SWFURL = 'http://www.rte.ie/static/player/swf/%s.swf' % match
        except:
            self.SWFURL=DEFAULTSWFURL
    ...
    def getVideoDetails(self, url, includeAds = True):
        ...
        soup = BeautifulSoup(page, selfClosingTags=['link','category','media:player','media:thumbnail','rte:valid', 'rte:duration', 'rte:statistics'])
        page.close()
        ...
        for content in contents:
            ...
            # FLV Advertisement
                ...
                soup = BeautifulSoup(page, selfClosingTags=['CustomParameters','ISCI'])
                page.close()
    ...
    def getEpisodes(self, combinedShowID):
        ...
        for item in items:
            try:
                show_id = item.find('showid').text
            except AttributeError:
                linkElement = item.find(attrs={'type' : 'application/atom+xml'})
                show_id = re.search('id=(\d+)', str(linkElement)).groups()[0]
            url = PLAYLIST_URL % (show_id, datetime.now().strftime('%Y%m%d%H%M'))
    ...
    def SearchAtoZ(self, letter = None):
        if letter == None or letter == '' or letter == 'Search by A-Z':
            for item in 'ABCDEFGHIJKLMNOPQRSTUVWXZ':
                yield {'Thumb':'https://raw.githubusercontent.com/Coolwavexunitytalk/images/master/rte%20icons/searchaz.png',
                       'Channel': CHANNEL,
                       'Title':item,
                       'url':MenuConstants.MODE_ATOZ,
                       'mode':MenuConstants.MODE_CREATEMENU,
